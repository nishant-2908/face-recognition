...
def image_to_encoding(filename):
    ...
    known_image = face_recognition.load_image_file(os.path.join(known_persons_directory, filename))
...
while True:
    ...
    for (top, right, bottom, left), name, confidence_score in zip(face_locations, face_names, confidence_scores):
        ...
        if confidence_score > 0:
            ...
            recognized_data = pd.concat([recognized_data, pd.DataFrame({"Name": [name], "Timestamp": [timestamp]})], ignore_index=True)
